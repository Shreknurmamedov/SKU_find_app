...
почему, текущее состояние и что дальше. Дата последнего обновления: 2026-06-19.
...
- **Новый live-guard на устройстве:** добавлен отдельный классификатор
  `weights/product_guard_cls.pt` (`interior` vs `product`) как второй этап после
  YOLO. Он отсекает стул/кулер/дверь/шкаф до попадания в `LiveCaptureTracker`.
  TFLite-asset экспортируется в CI как `product_guard_cls_float32.tflite`.
...
- `MainActivity` (CameraX, RGBA_8888) → `RgbFrame` → `TFLiteProductAnalyzer`
  ...
  imgsz 320) → candidate-боксы → `TFLiteProductGuard`
  (`assets/models/product_guard_cls_float32.tflite`, `interior` vs `product`) →
  live feedback / `LiveCaptureTracker`.
- Пороги в `TFLiteProductAnalyzer`: `CONF_THRESHOLD=0.55` (показ), `GREEN_CONF=0.70`,
  `MAX_AREA_FRAC=0.60` (отсечь стену/шкаф во весь кадр), `MIN_AREA_FRAC=0.004`.
- Пороги guard в `MainActivity`: `LIVE_GUARD_MIN_PRODUCT=0.55` для показа,
  `LIVE_GUARD_CAPTURE_PRODUCT=0.70` для статуса `Готово`.
...
1. **Собрать APK с `product_guard_cls_float32.tflite` через CI и проверить на
   планшете:** локальный TFLite-экспорт на macOS + Python 3.13 не работает, поэтому
   `weights/product_guard_cls.pt` должен быть закоммичен и экспортирован GitHub
   Actions. После установки APK повторить `adb screenrecord` в офисной комнате и на
   реальной полке.
2. **Ручная разметка (durable fix точности детектора):** нарезать ~200-300 кадров из
...
3. **Расширять эталонные фото SKU:** визуальный поиск уже подключен вторым сигналом
...
4. **Проверить v8_hardneg на SKU-presence CSV**: `weights/product_det_v8_hardneg.pt`
   ...
   `expected_sku_ids`.
5. **Облачный backend** (чтобы работало вне локальной сети; сейчас backend на Маке).
6. **Загружать кадры вместо видео** (или сжимать) — надёжнее по сети.
7. Подсчёт по одиночному фото (сейчас только видео).
...
### Live-привязка к товару на планшете (re-id, `docs/tablet-live-feedback.md`)
`LiveCaptureTracker` теперь привязывает feedback к конкретному товару устойчиво к
движению камеры: помимо IoU добавлена визуальная подпись кропа
(`MainActivity.signatureFromLuma`, 8×8 mean-removed L2-normalized luma, 64 числа)
и жадное сопоставление один-к-одному по IoU ИЛИ косинусу подписи. Уже снятый
товар, появившись в другом месте кадра, опознаётся и **не обводится повторно**;
вместо рамки рисуется зелёная ✓ (`ProductOverlayView.setCapturedMarks`). Подпись
замораживается при `captured`. Логика проверена симуляцией (re-id после ухода
камеры), `ProductOverlayView`/`ProductDetection`/`LiveCaptureTracker` собраны
javac со стабами; полная сборка APK — в CI.

### Ночной прогон самоулучшения (`docs/overnight-improve.md`)
Добавлены `ml/overnight_improve.py` + `scripts/run_overnight.sh` +
`ml/build_reference_cls_dataset.py`. Заполнить `data/eval/sku_presence.csv`
(реальные `sku_id` по 20-30 видео) → `./scripts/run_overnight.sh` → утром
`reports/overnight/SUMMARY.md`. Прогон: (1) перебор детектора (веса×conf) с
выбором лучшего по SKU-F1; (2) обучение визуального классификатора бренда на 14k
эталонных фото (8 классов, `yolo classify`), val top-1 — ориентир; (3) отчёт
«было/стало» + `best_config.json`. Все стадии изолированы и проверены end-to-end.
Классификатор в прод сам не подключается — решение по утреннему отчёту.

---

## 15. Сессия 2026-06-19 (Claude) — лог для продолжения в Codex

### Запушенные коммиты (origin/main, ветка main)
- `3474bff` SKU-presence audit + live feedback; политика **бренд+категория** (не
  угадывать модель, если артикул не прочитан) + багфиксы (импорт `sku_recognize`,
  `.gitignore` для индекса/датасетов).
- `af6bea1` Ночной прогон самоулучшения (`ml/overnight_improve.py`,
  `scripts/run_overnight.sh`, `ml/build_reference_cls_dataset.py`).
- `d7c009f` HTML-инструмент разметки (`ml/make_labeling_tool.py` →
  `reports/labeling/label_tool.html`, каталог встроен, поиск+клик → CSV).
- `64ea1b7` Планшет: привязка «снято» к товару через визуальный re-id
  (`ProductDetection.signature`, `LiveCaptureTracker`, ✓-метки в
  `ProductOverlayView`).
- `47aef73` Планшет: on-device отсев пустых стен/пола (бесцветный И плоский) +
  раскладка «камера занимает ~⅔ экрана».

Всё запушено и собрано в CI. Последняя сборка на планшете — **#28**
(`app-debug-11.apk`), установлена и проверена частично.

### Критичные операционные заметки по планшету (Galaxy Tab A9, SM-X115)
- **on-device модель = v6** (`weights/product_det_v2.pt` → tflite через
  `ml/export_tflite.py`, строка `WEIGHTS`). Негативы v7 откачены, v8_hardneg в APK
  не ставился. **Чтобы выпустить v8 на устройство:** поменять `WEIGHTS` в
  `ml/export_tflite.py` на `weights/product_det_v8_hardneg.pt`, push → CI.
- **Каталог (товар+конкуренты) НЕ обучает детектор** (он для распознавания SKU на
  бэкенде). Детектор «товар vs стена» учится на кадрах полок с ручными bbox +
  негативами. Это главная причина, почему «негативное обучение не помогло»: в APK
  его просто нет (v6), и каталог тут ни при чём.
- **Каждая CI-сборка подписана своим debug-ключом** → `adb install -r` поверх
  предыдущей падает (`INSTALL_FAILED_UPDATE_INCOMPATIBLE`). Надо:
  `adb uninstall com.utake.skufind` → `adb install <apk>` →
  `adb shell pm grant com.utake.skufind android.permission.CAMERA`.
  (Durable-фикс: фиксированный debug keystore через GitHub secret в CI.)
- **`screenrecord` пишет только в АЛЬБОМНОЙ** ориентации (в портрете —
  `ERROR: INVALID_LAYER_STACK`). `adb exec-out screencap` камеру не видит (чёрный
  кадр). Для захвата камеры+оверлея: форс альбомной
  (`adb shell settings put system accelerometer_rotation 0; ... user_rotation 1`),
  `adb shell screenrecord --time-limit 8 /sdcard/cap.mp4`, `adb pull`, кадры через
  cv2. **Восстановить авто-поворот:** `settings put system accelerometer_rotation 1`
  (сейчас оставлен форс-альбомный для съёмки).

### Что подтверждено на устройстве (#28)
Камера большая (раскладка ок); пустые стены НЕ обводятся (фильтр фона работает);
re-id «снято»+✓ работают; подсказки ближе/медленнее/наведите/держите работают.
Проверка делалась на офисной сцене — на реальной полке с товаром НЕ протестировано.

### Следующий шаг (открыто)
Навести планшет на **реальную полку с товаром** (Huter/Ресанта) в альбомной,
снять `screenrecord`, оценить выделение. Если **текстурная мебель/стеллажи** всё
ещё дают ложные боксы (фильтр ловит только плоский фон), то по приоритету:
1) выпустить v8_hardneg на устройство (меньше стен, но проверить recall); либо
2) разметить 200–300 кадров реальных полок (товар + негативы) и переобучить
   детектор — единственный надёжный фикс осцилляции FP↔FN.
Пороги фильтра тюнятся: `LIVE_BG_CHROMA_MAX`, `LIVE_BG_STD_MAX` в `MainActivity`.

---

## 16. Сессия 2026-06-19 (Codex): отдельный on-device guard `товар vs интерьер`

Запрос: настоящий фикс для планшета — не только пороги YOLO, а отдельная слабая
модель/классификатор на устройстве, который понимает `product` vs `interior`.

Сделано:
- добавлен `ml/build_product_guard_dataset.py`: строит ImageFolder-датасет
  `train/val × interior/product`;
- добавлен `ml/train_product_guard.py`: обучает YOLO classification, результат
  `weights/product_guard_cls.pt`;
- добавлен `ml/export_product_guard_tflite.py`: экспортирует guard в
  `mobile/android/app/src/main/assets/models/product_guard_cls_float32.tflite`;
- добавлен Android-класс `TFLiteProductGuard.java`;
- `MainActivity` теперь прогоняет каждый YOLO-crop через guard до live-оверлея и
  до `LiveCaptureTracker`;
- если guard asset отсутствует, приложение не падает и работает старой схемой
  YOLO + эвристики;
- `.github/workflows/android-apk.yml` экспортирует guard asset в CI, если
  `weights/product_guard_cls.pt` есть в репозитории;
- `.gitignore` разрешает коммитить только `weights/product_guard_cls.pt`, остальные
  веса остаются игнорируемыми;
- `docs/product-guard.md` описывает архитектуру, данные, команды и ограничения.

Данные обучения guard:
- positive: эталонные фото из `data/catalog/reference_dataset_all/training_images.csv`
  + реальные Huter-crop из `var/tablet/product_guard_positive`;
- negative: crop-ы комнаты/мебели/дверей/кулера из `var/tablet/sku_live_debug.mp4`,
  `var/tablet/cap4.mp4`, `var/tablet/cap_live.mp4`.

Факт обучения:
- датасет: примерно 1031 train / 181 val product и 1020 train / 180 val interior;
- обучен `weights/product_guard_cls.pt` (~3 МБ);
- локальный probe: 8/8 correct — Huter распознан как `product`, кулер/дверь/стул/
  шкаф как `interior`.

Проверки:
- `python3 -m py_compile` по новым/изменённым ML-файлам — OK;
- `git diff --check` — OK;
- искусственный тест `ml.evaluate_sku_coverage.row_metrics → aggregate_rows`
  подтвердил micro-average по видео: один SKU в неправильном видео = `1 FN + 1 FP`;
- backend pytest локально не запускался: в системном Python нет `pytest`;
- локальный TFLite export не запускать как критерий: macOS + Python 3.13 блокируется
  Ultralytics/TensorFlow Lite, экспорт должен пройти в GitHub Actions на Ubuntu.

Следующий обязательный шаг:
закоммитить `weights/product_guard_cls.pt` вместе с кодом, запустить GitHub Actions,
установить APK на Samsung Tab A9 и повторить live `screenrecord`. До этой проверки
нельзя утверждать, что проблема на планшете полностью решена.

---

## 17. Сессия 2026-06-19 (Codex): hard negatives + positives из планшета и `м9`

Запрос: подключить планшет, пройтись по офису, собрать hard negatives и positive
crop-ы; также использовать новую папку `м9` с фото из ТТ.

Сделано:
- с планшета записано raw-видео через само приложение:
  `var/tablet/collection_office_20260619/office_walk_raw.mp4`;
- из raw-видео намайнены crop-кандидаты в
  `var/tablet/collection_office_20260619/mined_crops`;
- папка `м9` сконвертирована в рабочие JPEG-копии:
  `var/photo_review_20260619/converted/м9`;
- добавлен reusable майнер `ml/mine_product_guard_crops.py`;
- по `м9` намайнено:
  - 1258 raw `positive_candidates`;
  - 2037 raw `hard_negative_candidates`;
  - 3016 `uncertain_review`;
- raw buckets не использовались напрямую: они отфильтрованы CLIP + image stats +
  визуальным контролем contact sheets;
- финально добавлено в тренировочные extra-папки:
  - `var/tablet/product_guard_positive`: 487 crop-ов;
  - `var/tablet/product_guard_negative`: 535 crop-ов;
- старые плохие `huter_room_004..011` удалены из positive-папки, потому что там
  были UI/пустая панель, а не чистый товар;
- `ml/build_product_guard_dataset.py` теперь умеет `--negative-extra-dir` и
  подхватывает curated hard negatives из `var/tablet/product_guard_negative`.

Новая сборка guard dataset:
- train: 3372 images;
- val: 594 images;
...
Результат:
- `weights/product_guard_cls.pt` обновлён;
- best validation top1: `0.995`;
- отчёт probe: `reports/product_guard_hardneg_probe_20260619.json`.

Сравнение old vs new guard на today's curated crop-ах:
- `m9_product` pass @0.75: old `21.7%` → new `98.1%`;
- `m9_interior` false accept @0.75: old `6.5%` → new `0.6%`;
- `office_product` pass @0.75: old `93.7%` → new `98.4%`;
- `office_interior` false accept @0.75: old `6.7%` → new `0.0%`.

Проверки:
- `python3 -m py_compile ml/build_product_guard_dataset.py ml/mine_product_guard_crops.py ml/train_product_guard.py` — OK;
- локальный TFLite export на macOS/Python 3.13 ожидаемо падает из-за ограничения
  Ultralytics/TensorFlow Lite. CI делает экспорт на Linux/Python 3.11.

Следующий шаг:
закоммитить код + `weights/product_guard_cls.pt`, дождаться CI APK, скачать и
